...
public class DeleteFaculty extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			b = DataManager.deleteFaculty(connection, facultyid);
			b1 = DataManager.deleteFacultySemInfo(connection, facultyid);
			if (b&b1) {
				JsonObject json = new JsonObject();
				json.addProperty("success", "1");
				json.addProperty("message", "faculty  deleted");
				response.getWriter().write(json.toString());
			} else if (!(b&b1)) {
				JsonObject json = new JsonObject();
				json.addProperty("success", "0");
				json.addProperty("message", "faculty  not delected");
				response.getWriter().write(json.toString());
			}
		} catch (Exception e) {
