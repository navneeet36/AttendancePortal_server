...
/**
 * Servlet implementation class StudentSemInfo
 */
@WebServlet("/StudentSemInfo")
public class StudentSemInfo extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public StudentSemInfo() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ArrayList<BeanStudentSemInfo> array = gson.fromJson(data_content,
					new TypeToken<ArrayList<BeanStudentSemInfo>>() {
					}.getType());
			boolean success = true;
			for (BeanStudentSemInfo bean : array)
				if (!DataManager.insertStudentSemInfo(connection, bean))
					success = false;
			if (success) {
