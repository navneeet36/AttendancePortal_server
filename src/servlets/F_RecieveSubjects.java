...
/**
 * Servlet implementation class F_RecieveSubjects
 */
@WebServlet("/F_RecieveSubjects")
public class F_RecieveSubjects extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public F_RecieveSubjects() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			ArrayList<BeanSubjectInfo> list=DataManager.receiveSubjects(connection);
			int count=0;
			if(list.size()>count)
			{	
				JsonObject json = new JsonObject();
					json.addProperty("success", "1");
					json.addProperty("message", "subject list for selection by faculty");
