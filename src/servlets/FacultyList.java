...
public class FacultyList extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			ArrayList<BeanFacultyInfo> list=DataManager.Facultylist(connection);
			int count=0;
			if(list.size()>count)
			{	
				JsonObject json = new JsonObject();
					json.addProperty("success", "1");
					json.addProperty("message", "faculty list is found ");
						json.addProperty("faculty_list",gson.toJson(list));
					...
					json.addProperty("message", "No faculty list found ");
					response.getWriter().write(json.toString());
				}
		} catch (Exception e) {
